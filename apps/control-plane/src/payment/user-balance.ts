...
export function deductBalance(wallet: string, amount: number): boolean {
    ...
    saveData(); // Persist to disk (fallback)

    // Sync to Supabase
    if (isSupabaseEnabled()) {
        upsertUserBalance(wallet, user.balance, user.totalDeposited, user.totalSpent)
            .catch(err => logger.warn('Database', `Failed to sync balance: ${err}`));
    }
...
export function creditBalance(wallet: string, amount: number, txHash?: string): void {
    ...
    if (isSupabaseEnabled()) {
        upsertUserBalance(wallet, user.balance, user.totalDeposited, user.totalSpent)
            .catch(err => logger.warn('Database', `Failed to sync balance: ${err}`));
        if (txHash) {
            recordDeposit(txHash, wallet, amount)
                .catch(err => logger.warn('Database', `Failed to record deposit: ${err}`));
        }
    }
